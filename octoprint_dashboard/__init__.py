...
class DashboardPlugin(
        ...
        if platform.system() == "Linux":
            ...
            if "cpu-thermal" in thermal:  # RPi
                ...
                if "sunxi-therm-2" in thermal:
                    ...
                    self.cpu_temp = max(
                        [
                            int(round(thermal["sunxi-therm-2"][0][1] * 1000)),
                            self.cpu_temp,
                        ]
                    )
    ...
    def run_cmd(self, cmd_index):
        ...
        self._plugin_manager.send_plugin_message(
            self._identifier, dict(cmdResults=json.dumps(results))
        )
    ...
    def test_cmd(self, cmd):
        ...
        if sys.version_info > (3, 5):
            # Python 3.5
            result = stdout.strip().decode("ascii", "ignore") + stderr.strip().decode(
                "ascii", "ignore"
            )
        else:
            # Python 2
        ...
        self._plugin_manager.send_plugin_message(
            self._identifier, dict(cmdTest=json.dumps(results))
        )
    ...
    def on_api_command(self, command, data):
        if command == "testCmdWidget":
            if (
                not ACCESS_PERMISSIONS_AVAILABLE
            ) or Permissions.PLUGIN_DASHBOARD_ADMIN.can():
                self._logger.info("testCmdWidget called, cmd is {cmd}".format(**data))
                self.test_cmd(data["cmd"])
            else:
                self._logger.info(
                    "testCmdWidget called, but without proper permissions"
                )
    ...
    def unload_preprocesser(self, processor, payload):
        ...
        self._logger.info(
            "GcodePreProcessor found layers: " + str(processor.layer_count)
        )
        self._logger.info(
            "GcodePreProcessor found filament changes: "
            + str(len(processor.filament_change_array))
        )
    ...
    def load_from_meta(self, payload):
        ...
        try:
            ...
            pass
        try:
            self.filament_change_array = json.loads(
                metadata["dashboard"]["filament_change_array"]
            )
        except KeyError:
            pass
        try:
        ...
        if int(self.total_layers) > 0:
            ...
            if payload["origin"] == "local":
                self._logger.info(
                    "Gcode not pre-processed by Dashboard. Processing now."
                )

                path = self._file_manager.path_on_disk(
                    FileDestinations.LOCAL, payload["path"]
                )
                file_object = octoprint.filemanager.util.DiskFileWrapper(
                    payload["name"], path
                )
    ...
    def print_ended(self):
        ...
        msg = dict(printEnd="True", layerAnalysisError=str(self.layer_analysis_error))
        self._plugin_manager.send_plugin_message(self._identifier, msg)
    ...
    def get_settings_defaults(self):
        ...
        return dict(
            ...
            enabledAnalysisModes=["cura", "simplify", "prusa", "prusa_old", "generic"],
            defaultWebcam=0,
    ...
    def on_settings_save(self, data):
        if (
            ACCESS_PERMISSIONS_AVAILABLE
            and not Permissions.PLUGIN_DASHBOARD_ADMIN.can()
        ):
            try:
    ...
    def process_gcode(
        ...
        if t - self.last_update > 0.5:
            ...
            msg = dict(
                ...
                extrudedFilament=str(
                    round(
                        (self.extruded_filament_store + self.extruded_filament) / 1000,
                        3,
                    )
                ),
            )
        ...
        if gcode in ("M117"):
            if cmd.startswith("M117 DASHBOARD_LAYER_INDICATOR"):
                ...
                self.current_layer = int(
                    cmd.replace("M117 DASHBOARD_LAYER_INDICATOR ", "")
                )
                self.layer_moves = 0
                self.layer_progress = 0
                ...
                if self.layer_start_time is not None and (
                    len(self.layer_labels) == 0
                    or self.current_layer - 1 > self.layer_labels[-1]
                ):
                ...
                self._plugin_manager.send_plugin_message(
                    self._identifier, dict(printerMessage=self.printer_message)
                )
            ...
            if matched:
                ...
                msg = dict(fanSpeed=str(self.fan_speed))
                self._plugin_manager.send_plugin_message(self._identifier, msg)
            ...
            if self.extruder_mode == "absolute":
                ...
                cmd_dict = dict(
                    (x, float(y))
                    for d, x, y in (re.split("([A-Z])", i) for i in cmd.upper().split())
                )
                if "E" in cmd_dict:
            ...
            self.layer_analysis_error[2] = (
                self.total_layers <= 0 or len(self.layer_move_array) <= 0
            )
            if (
                self.current_layer >= 0
                and self.total_layers > 0
                and len(self.layer_move_array) > 0
            ):
                try:
                    current_layer_progress = (
                        int(
                            (
                                self.layer_moves
                                / self.layer_move_array[self.current_layer]
                            )
                            * 100
                        )
                        if self.layer_move_array[self.current_layer] > 0
                        else 0
                    )
                except IndexError:
                    if not self.layer_analysis_error[1]:
                        self._logger.error(
                            "Error processing layer progress, IndexError: list index out of range"
                        )
                    self.layer_analysis_error[1] = True
                else:
                    self.layer_analysis_error[1] = False
                    if (
                        self.layer_moves % self.moves_to_update_progress == 0
                    ):  # Update the in, layer progress a reasonable amount
            ...
            if "E" in cmd_dict:
                ...
                if self.extruder_mode == "absolute":
                        ...
                        msg.update(
                            dict(
                                extrudedFilament=str(
                                    round(
                                        (
                                            self.extruded_filament_store
                                            + self.extruded_filament
                                        )
                                        / 1000,
                                        3,
                                    )
                    ...
                    if (
                        e > 10
                        or self.extruded_filament + e
                        > self.extruded_filament_last_send + 10
                    ):
                        self.extruded_filament += e
            ...
            if "Z" in cmd_dict:
                ...
                msg.update(dict(currentHeight=str(self.current_height)))
            if "F" in cmd_dict:
                now = time.time()

                # update the time weighted avg of feedrate
                self.avg_feedrate = (
                    self.avg_feedrate * (self.last_feed_change - self.feed_avg_start)
                    + self.current_feedrate * (now - self.last_feed_change)
    ...
    def create_file_pre_processor(self, path, file_object, *args, **kwargs):
        ...
        self.gcode_preprocessors[path] = GcodePreProcessor(
            ...
            self.filament_change_pattern,
            self.python_version,
            self._logger,
        )
        return octoprint.filemanager.util.StreamWrapper(
            file_name, self.gcode_preprocessors[path]
        )
...
def __plugin_load__():
    ...
    if ACCESS_PERMISSIONS_AVAILABLE:
        __plugin_hooks__["octoprint.access.permissions"] = (
